---
title: "R Basics – SPC 2025 Vanuatu Training"
format: 
  html:
    toc: true
    toc-depth: 3
...
my_numbers <- c(5, 10, 15, 20, 25, NA)
my_numbers_with_na <- c(5453, NA, 343, 850, NA, 9008)
...
# Q: can you now do the same (mean and max) using my_numbers_with_na?

```

## 4. Dataframe and Pipes `|>`

A dataframe is the same as a table!
...
# what if I want to see the total number of hooks by Yearmonth?
csv_data |>
  group_by(Yearmonth) |>
  summarise(total_n_hooks = sum(Hooks, na.rm = TRUE)) |>
  arrange(desc(total_n_hooks)) |>
  DT::datatable()

# Q: and what about if I want to know the mean value (average) of Sets by Yearmonth and Latitude?
